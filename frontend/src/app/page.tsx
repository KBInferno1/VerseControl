...
import { fetchStats, fetch1985Hymns, fetchNewHymns, fetchOriginalHymns, triggerScraper, triggerSeedPopulation, triggerDatabaseCleanup, Hymn1985, HymnNew, HymnOriginal } from '@/lib/api';
import { BookOpen, RefreshCw, Sparkles, Filter, Search, Layers, GitCompare, Database, Trash2, Music } from 'lucide-react';
import Link from 'next/link';

export default function CatalogPage() {
  const [stats, setStats] = useState<any>(null);
  const [activeTab, setActiveTab] = useState<'1985' | 'new' | 'original'>('1985');
  const [hymns1985, setHymns1985] = useState<Hymn1985[]>([]);
  const [hymnsNew, setHymnsNew] = useState<HymnNew[]>([]);
  const [hymnsOriginal, setHymnsOriginal] = useState<HymnOriginal[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (activeTab === '1985') {
        const data = await fetch1985Hymns(searchQuery, selectedTheme).catch(() => []);
        setHymns1985(data);
      } else if (activeTab === 'new') {
        const data = await fetchNewHymns(searchQuery, selectedTheme).catch(() => []);
        setHymnsNew(data);
      } else {
        const data = await fetchOriginalHymns(searchQuery, selectedTheme).catch(() => []);
        setHymnsOriginal(data);
      }
    } catch (e) {
      console.error(e);
    } finally {
  ...
  useEffect(() => {
    loadData();
  }, [activeTab, searchQuery, selectedTheme]);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-2 border-b border-gray-800 pb-2">
        ...
        >
          <BookOpen className="w-4 h-4" /> 1985 LDS Hymnal {stats ? `(${stats.count_1985})` : ''}
        </button>
        ...
        >
          <Sparkles className="w-4 h-4" /> New Digital Releases {stats ? `(${stats.count_new})` : ''}
        </button>
        ...
        >
          <Music className="w-4 h-4" /> Traditional Christian Precursors {stats ? `(${stats.count_original})` : ''}
        </button>
      </div>
      ...
      <div className="bg-lds-cardBg border border-gray-800 p-6 rounded-xl space-y-4">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="relative w-full sm:w-96">
            ...
            <input
              type="text"
              placeholder={`Search ${activeTab === '1985' ? '1985 hymns' : activeTab === 'new' ? 'new digital hymns' : 'traditional precursors'} by title or lyrics...`}
              value={searchQuery}
        ...
        {loading ? (
          ...
          hymns1985.length === 0 ? (
            <div className="py-12 text-center text-gray-500 text-sm">No 1985 hymns found matching filter.</div>
          ) : (
              ...
              {hymns1985.map((hymn) => (
                <div key={hymn.id} className="bg-slate-900/60 border border-gray-800 hover:border-lds-accent/50 p-5 rounded-xl transition-all space-y-3 flex flex-col justify-between">
                  ...
                  <div className="pt-3 border-t border-gray-800/60 flex items-center justify-between">
                    <span className="text-xs text-gray-500">1985 Hymnal Print</span>
                    <Link
          ...
          hymnsNew.length === 0 ? (
            <div className="py-12 text-center text-gray-500 text-sm">No new digital hymns found matching filter.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-4">
              {hymnsNew.map((hymn) => (
                <div key={hymn.id} className="bg-slate-900/60 border border-emerald-500/20 hover:border-emerald-400/50 p-5 rounded-xl transition-all space-y-3 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-300 font-mono">
                        {hymn.batch_release || 'Batch 1'}
                      </span>
                    </div>
                    <h3 className="font-bold text-white text-base">{hymn.title}</h3>
                    {hymn.major_theme && (
                      <span className="text-xs text-gray-400 bg-slate-950 px-2 py-0.5 rounded border border-gray-800 inline-block mt-1">
                        {hymn.major_theme}
                      </span>
                    )}
                    <p className="text-xs text-gray-400 font-serif line-clamp-3 mt-2 leading-relaxed">
                  ...
                  <div className="pt-3 border-t border-gray-800/60 flex items-center justify-between">
                    <span className="text-xs text-emerald-400 font-medium">New Release</span>
                    <Link
                      href={`/compare?hymn=${hymn.hymn_number}`}
          ...
          hymnsOriginal.length === 0 ? (
            <div className="py-12 text-center text-gray-500 text-sm">No traditional Christian precursor hymns found matching filter.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-4">
              {hymnsOriginal.map((hymn) => (
                <div key={hymn.id} className="bg-slate-900/60 border border-amber-500/20 hover:border-amber-400/50 p-5 rounded-xl transition-all space-y-3 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-semibold text-amber-300 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20">
                        {hymn.original_author || 'Traditional Author'}
                      </span>
                      {hymn.publication_year && (
                    ...
                    <h3 className="font-bold text-white text-base">{hymn.title}</h3>
                    {hymn.original_source && (
                      <p className="text-xs text-amber-400/70 italic mt-0.5">{hymn.original_source}</p>
                    )}
                    <p className="text-xs text-gray-400 font-serif line-clamp-3 mt-2 leading-relaxed">
                      {hymn.lyrics}
                    </p>
                  ...
                  <div className="pt-3 border-t border-gray-800/60 flex items-center justify-between">
                    <span className="text-xs text-amber-400 font-medium">Christian Original</span>
                    <Link
                      href="/compare"
